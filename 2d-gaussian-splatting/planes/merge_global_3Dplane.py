...
def vis_plane_mask(plane_mask, plane_id, rgb_path, save_path):
    ...
    Vis plane mask
    """
    # Ensure plane_mask is on CPU for visualization
    if isinstance(plane_mask, torch.Tensor):
        plane_mask = plane_mask.cpu()
        
    obj_mask_map = (plane_mask == plane_id)
    rgb = Image.open(rgb_path)
    rgb = np.array(rgb)
    mask_rgb_map = np.zeros_like(rgb)
    obj_mask_map_temp = obj_mask_map.numpy()
...
def vis_plane_mask_1(plane_mask, plane_id, rgb_path, save_path, transparency=0.6, color=[0, 0, 255]):
    ...
    """
    # Ensure plane_mask is on CPU
    if isinstance(plane_mask, torch.Tensor):
        plane_mask = plane_mask.cpu()

    # Create binary mask for the specific plane
    obj_mask_map = (plane_mask == plane_id)
    obj_mask_map = obj_mask_map.numpy()
...
def get_plane_pnts_idx_from_mask(pixel_pnts_map, plane_mask, plane_id):
    ...
    """
    # plane_mask and pixel_pnts_map should be on same device (GPU) here
    obj_mask_map = (plane_mask == plane_id)
...
def get_global_3Dplane(cameras, points, plane_masks, previous_global_3Dplane_ID_dict):
    ...
    if previous_global_3Dplane_ID_dict is None:
        ...
        # OPTIMIZATION: Move mask to GPU only when needed
        plane_mask = plane_masks[cur_view_id].cuda()
        ...
        for global_3Dplane_id, local_3Dplane_id_list in global_3Dplane_ID_dict.items():
            ...
            for view_id, plane_id in local_3Dplane_id_list:
                # OPTIMIZATION: Move mask to GPU only when needed
                plane_mask = plane_masks[view_id].cuda()
                plane_pnts_idx = get_plane_pnts_idx_from_mask(pixel_pnts_map_list[view_id], plane_mask, plane_id)
                ...
                del plane_mask # clean up
            global_3Dplane_pnts_idx.append(global_3Dplane_pnts_idx_i)
        
        # Clean up list of maps
        del pixel_pnts_map_list

    for view_id in range(cur_view_id+1, len(cameras)):
        # OPTIMIZATION: Explicit cache clear to prevent fragmentation
        torch.cuda.empty_cache()
        
        pixel_pnts_map = get_pixel_to_points_tensor(cameras[view_id], points)
        
        # Move current mask to GPU
        plane_mask = plane_masks[view_id].cuda()
...
if __name__ == "__main__":
    # OPTIMIZATION: Enable no_grad globally to save memory
    with torch.no_grad():
        ...
        plane_masks = []
        print("Loading plane masks to CPU memory...")
        for i in range(len(train_viewpoints)):
            ...
            # OPTIMIZATION: Keep masks on CPU until needed
            plane_mask = torch.tensor(plane_mask, dtype=torch.int).cpu() 
        ...
        for refine_points_file_name_i in refine_points_file_name:
            ...
            # OPTIMIZATION: Load to CPU first
            pnts_i = torch.tensor(pnts_i, dtype=torch.float32).cpu()
        ...
        if len(pnts_list) == 0:
            ...
            # OPTIMIZATION: Cat on CPU then move to GPU once
            print("Concatenating points and moving to GPU...")
            pnts = torch.cat(pnts_list, dim=0).cuda()
            del pnts_list # free CPU memory list
            gc.collect()
            print(f'********** load refine points: {pnts.shape[0]} points **********')
        ...
        # Clean cache before heavy operation
        torch.cuda.empty_cache()
        ...
        if args.vis_plane_path is not None:
            ...
            for idx in range(len(global_3Dplane_pnts_idx)):
                ...
                for view_id, plane_id in global_3Dplane_ID_dict_i:
                    ...
                    # Pass mask (CPU or GPU handled by vis function)
            ...
            for idx in range(len(train_viewpoints)):
                ...
                plane_mask = plane_masks[idx].cuda() # move to GPU for processing
                plane_id_list = torch.unique(plane_mask)
                for plane_id in plane_id_list:
                    if plane_id == 0:
                        continue
                    ...
                    # vis using cpu mask from list
                    vis_plane_mask(plane_masks[idx], plane_id, rgb_path, os.path.join(local_save_root_path, f'frame{idx:06d}_plane{plane_id:06d}_mask.png'))
                    print(f'plane{plane_id} mask saved')
